...
module.exports = {
  ...
  EditProfile: async (req, res) => {
    try {
      ...
      const hostUpdate = await Models.userModel.findOne({
        where: { id: hostId },
      });
      return res
        .status(200)
        .json({ message: "HOST PROFILE EDITED!", hostUpdate });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "ERROR", error });
    }
  ...
  AccountDeleted: async (req, res) => {
    try {
      ...
      const deleteHost = await Models.userModel.destroy({
        where: { id: hostId },
      });
      return res
        .status(200)
        .json({ message: "HOST ACCOUNT DELETED!", deleteHost });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "ERROR", error });
    }
  ...
  fetchSingleBooking: async (req, res) => {
    try {
      const { id } = req.body;
      const booking = await Models.bookingModel.findOne({ id });
      if (!booking) {
        return res.status(404).json({ message: "BOOKING NOT FOUND" });
      }
  ...
  ActivityAdded: async (req, res) => {
    try {
      ...
      const activityAdded = await Models.activityModel.create({
        DateAndTime,
        Duration,
        hourlyRate,
        Tags,
      ...
      const activityImage = await Models.activityImages.create({
        activityId: activityAdded.id,
        uploadImage: path,
        GovtId: path1,
      });
      return res
        .status(200)
        .json({ message: "ACTIVITY ADDED!", activityAdded, activityImage });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "ERROR", error });
    }
  ...
  editActivity: async (req, res) => {
    try {
      ...
      const path = await commonHelper.fileUpload(file);
      const path1 = await commonHelper.fileUpload(file1);
      const activityUpdate = await Models.activityModel.update(
        {
          DateAndTime,
          Duration,
          hourlyRate,
      ...
      const activityImage = await Models.activityImages.update(
        { uploadImage: path, GovtId: path1 },
        { where: { id: activityId } },
      );
      return res
        .status(200)
        .json({ message: "ACTIVITY UPDATED!", activityUpdate, activityImage });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "ERROR", error });
    }
  ...
  deleteActivity: async (req, res) => {
    try {
      ...
      const deleteActivity = await Models.activityModel.destroy({
        where: { activityId },
      });
      return res
        .status(200)
        .json({ message: "ACTIVITY DELETED!", deleteActivity });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "ERROR", error });
    }
  ...
  BankDetails: async (req, res) => {
    try {
      ...
      const BankDetails = await Models.transactionModel.create({
        BankName,
        BranchName,
        AccountNumber,
        IFSC,
      ...
      return res
        .status(200)
        .json({ message: "BANK DETAILS ADDED:", BankDetails });
    } catch (error) {
      console.log(error);
  ...
  UserDetails: async (req, res) => {
    try {
      ...
      const userDetails = await Models.userModel.findOne({
        where: { id: userId },
        include: [
          {
