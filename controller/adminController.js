     ...
     adminLogin: async (req, res) => {
      ...
      EditAdminProfile:async(req,res)=>
      {
        try
        {
           ...
           const adminUpdate=await Models.userModel.findOne({where:{id:adminId}})
           return res.status(200).json({message:"ADMIN PROFILE EDITED!",adminUpdate})
        }
        catch(error)
        {
           console.log(error)
      ...
      adminAccountDeleted:async(req,res)=>
      {
        try
         { 
           ...
           const deleteadmin=await Models.userModel.destroy({where:{id:adminId}})
           return res.status(200).json({message:"ADMIN ACCOUNT DELETED!",deleteadmin})
         }
         catch(error)
         {
          console.log(error)
       ...
       faqAndHelp:async(req,res)=>
        {
         ...
         subscriptionCreated:async(req,res)=>
         {
          try
          {
            ...
            return res.status(200).json({message:"SUBSCRIPTION CREATED!",subCreated})
          }
          catch(error)
          {
            console.log(error)
         ...
         subscriptionEdit:async(req,res)=>
         {
          try
          {
            const{subscriptionId,title,Amount,description,subscriptionType}=req.body;
            ...
            if(!sub)
            {
              ...
              await Models.subscriptionModel.update({title,Amount,description,subscriptionType},{where:{id:subscriptionId}})
            const updatedSubscription=await Models.subscriptionModel.findOne({where:{id:subscriptionId}})
         ...
         subscriptionDeleted:async(req,res)=>
         {
          try
          {
           ...
           const deleteSub=await Models.subscriptionModel.destroy({where:{id:subscriptionId}})
           return res.status(200).json({message:"SUBSCRIPTION DELETED!",deleteSub})
          }
          catch(error)
          {
            console.log(error)
